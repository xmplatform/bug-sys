...
<body>
	...
	<div class="container-fluid breadcrumb">
		<div class="row-fluid span12">
			<span class="span4">项目名称: <b>${bugProject.name}</b></span>
			<span class="span4">项目经理: ${bugProject.createBy.name}</span>
		</div>
	...
	<div class="breadcrumb">
		<form id="assignRoleForm" action="${ctx}/bug/bugProject/assignBugProject" method="post" class="hide">
			<input type="hidden" name="id" value="${bugProject.id}"/>
		...
		top.layer.open({
		    ...
		    content: "${ctx}/bug/bugProject/usertobugProject?id=${bugProject.id}" ,
		    btn: ['确定', '关闭'],
		...
		<c:forEach items="${userList}" var="user">
			<tr>
				...
				<shiro:hasPermission name="sys:role:edit"><td>
					<a href="${ctx}/bug/bugProject/outUser?userId=${user.id}&projectId=${bugProject.id}"
						onclick="return confirmx('确认要将用户<b>[${user.name}]</b>从<b>[${bugProject.name}]</b>项目中移除吗？', this.href)">移除</a>
